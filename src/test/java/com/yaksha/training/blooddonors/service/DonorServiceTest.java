...
public class DonorServiceTest {
    ...
    @Test
    public void testServiceSearchDonorWithNull() throws Exception {
        List<Donor> actual = getDonorList(5);
        when(donorRepository.findAll()).thenReturn(actual);
        List<Donor> expected = donorService.searchDonors(null);
        yakshaAssert(currentTest(),
                (asJsonString(expected).equals(asJsonString(actual))
                        ? "true"
                        : "false"),
    ...
    @Test
    public void testServiceSearchDonorWithSearchName() throws Exception {
        String searchKey = randomStringWithSize(2);
        List<Donor> actual = getDonorList(5);
        when(donorRepository.findByStateAndCity(searchKey)).thenReturn(actual);
        List<Donor> expected = donorService.searchDonors(searchKey);
        yakshaAssert(currentTest(),
                (asJsonString(expected).equals(asJsonString(actual))
                        ? "true"
                        : "false"),
